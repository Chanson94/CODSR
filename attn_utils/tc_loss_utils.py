...
def get_grounding_loss_by_list(_gt_seg_list, input_attn_map_ls):
    """
        _gt_seg_list (List[Tensor]): (1, 1, res, res)
        input_attn_map_ls (List[Tensor]): (b*head, res, res, 77), sum = b*head*res*res
    ...
    bce_loss_func = nn.BCELoss()
    pixel_loss = 0.0
    C, H, W = input_attn_map_ls[0].shape
    for i in range(len(gt_seg_list)):
        if gt_seg_list[i].sum() > 0.1:

            ################### token loss start ###################
            mask = gt_seg_list[i]
            ca_map_obj = input_attn_map_ls[i]
            # print("mask: ", mask.requires_grad)
            # print("ca_map_obj: ", ca_map_obj.requires_grad)
            # calculate pixel value inside mask
            activation_value = (ca_map_obj * mask).reshape(1, -1).sum(dim=-1)/ca_map_obj.reshape(1, -1).sum(dim=-1)
            obj_loss = (1.0 - torch.mean(activation_value)) ** 2
            token_loss += obj_loss
    ...
    # average with len word_token_idx_ls
    if  valid_gt_seg > 0 :
        # print("valid_gt_seg_pixel:", valid_gt_seg)
        # print("token_loss:",token_loss, token_loss / valid_gt_seg)
        token_loss = token_loss / valid_gt_seg
        # print("pixel_loss:",pixel_loss, pixel_loss  / valid_gt_seg)
        pixel_loss = pixel_loss  / valid_gt_seg 
