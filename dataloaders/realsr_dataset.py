...
class PairedSROnlineTxtDataset(torch.utils.data.Dataset):
    def __init__(self, split=None, args=None):
        ...
        if split == 'train':
            ...
            self.gt_list = []
            assert len(args.dataset_txt_paths_list_val) == len(args.dataset_prob_paths_list_val)
            for idx_dataset in range(len(args.dataset_txt_paths_list_val)):
                with open(args.dataset_txt_paths_list_val[idx_dataset], 'r') as f:
                    dataset_list = [line.strip() for line in f.readlines()]
                    for idx_ratio in range(args.dataset_prob_paths_list_val[idx_dataset]):
                        gt_length = len(self.gt_list)
                        self.gt_list += dataset_list
                        print(f'=====> append {len(self.gt_list) - gt_length} data.')
    def __len__(self):
