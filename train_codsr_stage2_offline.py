...
from dataloaders.Realesrgan_offline_all_HQ_dataset_stage2 import LocalImageDataset_selectedv2_offline
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help="Root directory of offline SAM2 masks generated by scripts/preprocess_stage2_masks.py",
    )
...
def collate_fn(data):
    ...
    words = [example["words"] for example in data]
    masks = [example["masks"] for example in data]
...
def generate_attention_map(per_layer_heatmaps, layer_name, x_src):
    ...
    B, C, H, W = x_src.shape
    heat_up = F.interpolate(heat.unsqueeze(1), size=(H, W), mode="bilinear", align_corners=False)[0, 0]
    return heat_up
...
def main(args):
    ...
    train_dataset = LocalImageDataset_selectedv2_offline(
        img_file=[img_file_path, lq_img_file_path],
    ...
    model_gen, model_reg, optimizer, optimizer_reg, dl_train, dl_val, lr_scheduler, lr_scheduler_reg = accelerator.prepare(
        model_gen, model_reg, optimizer, optimizer_reg, dl_train, dl_val, lr_scheduler, lr_scheduler_reg
    )
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*m_acc):
                ...
                batch["prompt"] = list(batch["prompt"])
                clean_words_list = list(batch["words"][0])
                mask_list = batch["masks"][0]

                attn_maps = []

                x_tgt_pred, latents_denoised, prompt_embeds, neg_prompt_embeds = model_gen(
                    x_src.detach(), prompt=batch["prompt"], num_inference_steps=1, args=args
                )
                ...
                if len(clean_words_list) != 0:
                    mask_list_on_device = [
                        mask.unsqueeze(0).to(accelerator.device, dtype=torch.float32)
                        for mask in mask_list
                    ]
                    if len(mask_list_on_device) == len(attn_maps):
                ...
                if torch.cuda.device_count() > 1:
                    loss_kl = model_reg.module.distribution_matching_loss(
                        latents=latents_denoised, prompt_embeds=prompt_embeds, neg_prompt_embeds=neg_prompt_embeds, args=args
                    ) * lambda_vsd
                else:
                    loss_kl = model_reg.distribution_matching_loss(
                        latents=latents_denoised, prompt_embeds=prompt_embeds, neg_prompt_embeds=neg_prompt_embeds, args=args
                    ) * lambda_vsd
