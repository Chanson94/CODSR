...
def main():
    ...
    # input_tensors = torch.randn((inference_iterations, batch_size, 3, 768, 576), device=args.device, dtype=weight_dtype)
    ...
    flops_analyzer = FlopCountAnalysis(model, (lq_processed, validation_prompt))
    flops = flops_analyzer.total() / 1e12 
    print(f"FLOPs: {flops} T FLOPs")
