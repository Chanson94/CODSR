    ...
    for dir_idx, init_dir in enumerate(args.inp_imgs):
        ...
        for img_idx, sr_path in enumerate(img_sr_list):
            ...
            if sr_img.shape != gt_img.shape:
                print(f"sr_img.shape:{sr_img.shape}, gt_img.shape:{gt_img.shape}")
                sr_img = cv2.resize(
                    sr_img, 
                    dsize=(gt_img.shape[1], gt_img.shape[0]),               # target size (width, height)
            ...
            with torch.no_grad():
                ...
                for name, metric in iqa_metrics.items():
                    if name in ['CLIPIQA', 'CLIPIQA+', 'NIQE', 'MUSIQ', 'MANIQA']:
